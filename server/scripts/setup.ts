 ...
 * 1. Provision environment variables (.env file)
...
const ENV_EXAMPLE_FILE = resolve(import.meta.dirname, "../.env.example");
...
function provisionEnv(): void {
  console.log("🔧 Provisioning environment variables...\n");
  ...
  if (!existingEnv.DATABASE_URL && !process.env.DATABASE_URL) {
    ...
    console.log(`   ✓ Set DATABASE_URL (default: ${DEFAULTS.DATABASE_URL})`);
  } else {
  ...
  if (additions.length > 0) {
    ...
    console.log(`\n   📝 Added ${additions.length} variable(s) to .env`);
  }
